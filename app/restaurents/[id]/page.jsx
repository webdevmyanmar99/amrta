...
async function fetchRestaurentDetail(id) {
  const restaurentdetail = await fetch(`http://139.59.246.244/place/${id}`, {
    cache: "no-store",
  });
  return await restaurentdetail.json();
}

export default async function RestaurentDetails({ params }) {
  const data = await fetchRestaurentDetail(params.id);
